...
export const arkmeForwardStyles: Record<string, CSSProperties> = {
  ...
  forwardTargetSearchWrap: { margin: '4px 18px 12px', height: 38, flex: 'none' },
...
export function ArkmeForwardTargetRow({ target, selected, disabled, meta, statusText, onToggle }: {
  ...
  return <button data-arkme-feedback="neutral" type="button" style={{ ...styles.forwardTargetRow, ...(selected ? styles.forwardTargetRowSelected : {}) }}
...
export function ArkmeForwardDialog({ children, dialogRef, viewport = false, keyword, onKeywordChange, sending,
  ...
  return <div data-arkme-notification-blocking-overlay="true" style={{ ...styles.forwardTargetBackdrop, ...(viewport ? { position: 'fixed', zIndex: 1750 } : {}) }}
    ...
    <section ref={dialogRef} style={styles.forwardTargetDialog} role="dialog" aria-modal="true" aria-label={tr("选择转发对象")} aria-labelledby={titleId}>
      <header style={styles.forwardTargetHeader}><span aria-hidden /><h3 id={titleId} style={styles.forwardTargetTitle}>{tr("转发给")}</h3>
        <button data-arkme-feedback="neutral" type="button" style={styles.forwardTargetClose} disabled={sending} aria-label={tr("关闭转发对象选择")} onClick={onClose}>×</button>
      </header>
      <div style={styles.forwardTargetSearchWrap}>
        <ArkmeForwardSearch value={keyword} disabled={sending} onChange={event => onKeywordChange(event.currentTarget.value)} />
      </div>
      <div style={styles.forwardTargetBody}>{children}</div>
      {selectedTargets.length > 0 && <footer style={styles.forwardTargetFooter}>
        <div style={styles.forwardTargetRecipients}><span>{tr("发送给：")}</span><span style={styles.forwardTargetAvatarStack}>
        ...
        <div data-arkme-forward-source="true" style={styles.forwardTargetPreview}>
          ...
          <button data-arkme-feedback="neutral" type="button" style={styles.forwardTargetPreviewClose} disabled={sending} aria-label={tr("关闭转发对象选择")} onClick={onClose}><ArkmeForwardCloseBorderIcon /></button>
        </div>
        {error && <div role="alert" style={styles.forwardTargetSendError}>{error}</div>}
        <div style={styles.forwardTargetComposer}>
          <textarea style={styles.forwardTargetCommentInput} value={comment} placeholder={tr("说点什么...")} aria-label={tr("转发附言")} disabled={sending || commentDisabled}
          ...
          <button data-arkme-feedback="primary" type="button" style={{ ...styles.forwardTargetSend, ...(sending ? styles.forwardTargetSendDisabled : {}) }} disabled={sending}
            aria-label={sending ? tr("转发中") : tr("发送转发")} onClick={onSend}><ArkmeForwardSubmitIcon /></button>
        </div>
      </footer>}
    </section>
  </div>
