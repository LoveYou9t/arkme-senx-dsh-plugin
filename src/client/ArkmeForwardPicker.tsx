...
function errorMessage(error: unknown): string {
  return error instanceof ArkmeClientError ? error.body.message : error instanceof Error ? error.message : tr("转发失败，请重试")
}
function targetMeta(source: ArkmeSourceItem): string {
  return source.kind === 'private_chat' ? tr("私聊") : source.kind === 'group_chat' ? tr("群聊") : source.kind === 'topic' ? tr("主题") : tr("发给自己")
}
...
export function ArkmeForwardPicker({ open = true, source, messageCount, delivery, onClose, onComplete, onStatus, onForwarded }: {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (failures.length || warnings.length) {
        ...
        const message = failures.length ? successCount ? tr("已转发 {v0} 个目标，{v1} 个失败，可重试", { v0: String(successCount), v1: String(failures.length) }) : request.signal.aborted ? '转发超时，请使用原请求重试' : errorMessage(rejected?.reason) : warning?.value.warningText ?? '转发已完成，附言发送失败'
        setError(message)
        onStatus(failures.length && successCount ? tr("已转发 {v0} 个目标，{v1} 个失败", { v0: String(successCount), v1: String(failures.length) }) : message)
      } else {
        onStatus(tr("已转发到 {v0} 个目标", { v0: String(completed.current.size) }))
        onComplete()
      }
    } finally {
  ...
  return <div data-arkme-forward-picker="true" onKeyDown={event => {
    ...
    <ArkmeForwardDialog dialogRef={dialog} viewport keyword={keyword} onKeywordChange={setKeyword} sending={sending}
      ...
      previewIcon={source?.icon} previewTitle={source?.title ?? tr("聊天记录")}
      previewSubtitle={source?.subtitle ?? (messageCount === undefined ? tr("聊天记录") : `${messageCount} 条消息`)}
      ...
      <ul style={styles.forwardTargetList} aria-label={tr("转发对象列表")}>
        {[...filtered.filter(target => target.kind === 'send_to_self'), ...filtered.filter(target => target.kind !== 'send_to_self')].map(target => {
          ...
          return <li key={key}><ArkmeForwardTargetRow target={target} selected={checked}
            ...
            {...(completed.current.has(key) ? { statusText: tr("已转发") } : {})} onToggle={() => {
              if (busy.current || completed.current.has(key)) return
              if (!checked && selected.length >= 5) { onStatus(tr("最多选择 5 个转发对象")); return }
